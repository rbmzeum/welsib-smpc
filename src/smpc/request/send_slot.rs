...
#[derive(Debug)]
pub struct SendSlotRequestAttributes {
...
impl SendSlotRequestAttributes {
    pub fn new(slot_type: SlotType, slot_index: usize, slot_bytes: Vec<u8>, client_index: usize, nonce_sig: String, keypair: &Keypair) -> Self {
    ...
    pub fn from_json(json: &str) -> Option<Self> {
        ...
        if let JsonValue::Object(obj) = from_json(json.as_str()).unwrap() {
            ...
            let slot_bytes = if let Some(JsonValue::Array(slot_bytes)) = obj.get("slot_bytes") {
                slot_bytes.iter().map(|j_val| if let JsonValue::Number(v) = j_val { v.clone() as u8 } else { 0 /* NB! */ } ).collect::<Vec<u8>>()
            } else {
                return None;
            };
    ...
    pub fn to_json(&self) -> String {
        ...
        obj.insert(String::from("slot_bytes"), JsonValue::Array(self.slot_bytes.clone().iter().map(|v| { JsonValue::Number(v.clone() as u64) }).collect::<Vec<_>>()));
...
// impl WelsibDtoInterface for SendSlotRequestAttributes {}