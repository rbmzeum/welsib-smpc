...
fn parse_u512_from_line(line: &str) -> Option<U512> {
    // Ищем паттерн U512([
    if let Some(start) = line.find("U512([") {
        let sub = &line[start + 6..]; // Пропускаем "U512(["
...
fn parse_point_from_line(line: &str) -> Option<Point> {
    if let Some(x) = parse_u512_from_line(line) {
        // В логе показываются только x координаты, y устанавливаем в 0
...
fn parse_debug_log(path: &str) -> io::Result<ParsedData> {
    ...
    for line_result in reader.lines() {
        ...
        // Определяем тип клиента по его публичному ключу
        if line.contains("от клиента U512([") {
            if let Some(client_start) = line.find("от клиента U512([") {
                let client_part = &line[client_start + 12..]; // "от клиента " имеет 12 символов в UTF-8
                if let Some(x) = parse_u512_from_line(client_part) {
                    let client_id = format!("{:?}", x);
        ...
        if line.contains("Получена point_list от клиента") && line.contains(": U512([") {
            if let Some(colon_idx) = line.find(": U512([") {
                let point_str = &line[colon_idx + 2..]; // ": " имеет 2 символа
                if let Some(point) = parse_point_from_line(point_str) {
                    // Определяем, от какого клиента
                    if let Some(client_start) = line.find("от клиента U512([") {
                        let client_part = &line[client_start + 12..];
                        if let Some(client_x) = parse_u512_from_line(client_part) {
        ...
        if line.contains("Получена point_matrix от клиента") && line.contains(": U512([") {
            if let Some(colon_idx) = line.find(": U512([") {
                let point_str = &line[colon_idx + 2..];
                if let Some(point) = parse_point_from_line(point_str) {
                    if let Some(client_start) = line.find("от клиента U512([") {
                        let client_part = &line[client_start + 12..];
                        if let Some(client_x) = parse_u512_from_line(client_part) {
        ...
        if line.contains("Точка из bit_proofs для клиента") && line.contains("= U512([") {
            if let Some(equal_idx) = line.find("= U512([") {
                ...
                if let Some(point) = parse_point_from_line(point_str) {
                    if let Some(client_start) = line.find("для клиента U512([") {
                        let client_part = &line[client_start + 15..];
                        if let Some(client_x) = parse_u512_from_line(client_part) {
                            let client_id = format!("{:?}", client_x);
                            if let Some(&client_idx) = client_map.get(&client_id) {
                                match client_idx {
        ...
        if line.contains("matrix_controller_point = U512([") {
            if let Some(point) = parse_point_from_line(&line) {
                data.mc = Some(point);
            }
        }
